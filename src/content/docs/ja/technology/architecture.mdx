...
- **Local Inference**: 実行ログ、ソースコード、スクリーンショットなどの巨大なデータをクラウド LLM に送る前に、ローカル NPU 上の軽量エッジモデルが一次解析（蒸留）を行い、不要なデータを削ぎ落します。
- **Token Efficiency**: 解析結果の「インサイト（結論）」のみをクラウドへ送ることで、トークン費用を最大 1/100 まで削減します。
- **Eco-System**: 通信量を最小化することで、ネットワーク負荷を抑え、オフラインに近い感覚で高速にフィードバックを得られます。
