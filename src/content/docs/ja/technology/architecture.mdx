...
Aether Lens は、2026年時点でのハードウェアの進化、特に **NPU (Neural Processing Unit)** による「真の低コスト開発」に最適化されています。

- **Token Efficiency (via Layered Context)**: 解析結果の「インサイト（結論）」のみを優先的に MCP 経由で AI へ送ることで、トークン費用を最大 1/100 まで削減します。詳細は [Layered Context via MCP](#layered-context-via-mcp) を参照。
- **Eco-System**: データ転送量を最小化することで、ネットワーク負荷を抑え、高速なフィードバックを得られます。
- **Autonomous Hardware Detection**: 2026年標準の NPU 命令セットをバイナリレベルでサポート。設定ファイルでの指定は不要で、ハードウェアの存在を自動検知して最適化パス（蒸留プロセス）を選択します。NPU がない環境では、透過的に CPU/GPU 処理にフォールバックします。
...
2. **Shadow Verification (Core Default)**: 開発者がコードを保存した瞬間、バックグラウンドで自律的に検証を開始。設定不要で常に「作業の正しさ」を監視し続けます。
