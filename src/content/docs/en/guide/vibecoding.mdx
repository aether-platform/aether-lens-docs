...
Aether Lens allows you to flexibly switch where tests are executed. By offloading tests to container environments, you minimize the load on your local machine.

- **Local**: For fast unit tests and lightweight script execution.
- **Docker**: Executes complete tests, including dependencies, in an environment close to production.
- **Kubernetes**: Leverages the cluster power of Aether Platform for large-scale parallel testing.
...
Aether Lens functions as a **Unified Aggregation Layer** that bundles execution results from all types of tests and tasks.

Frontend VRT results and backend unit test results are all processed as equivalent "events." This centrally visualizes the quality status of the entire project, independent of specific languages or frameworks.
