...
| **Custom** | User Defined | Specialized analysis via `custom_instruction`. |
...
Aether Lens functions as an **Aggregation Layer** that bundles tests from different layers. It acts not just as a test runner, but as a conductor for various tools (Jest, Playwright, Backend Check).

- **Layered Execution**: Runs Unit (Logic), Integration (API), and E2E (Visual) tests in parallel or in order of dependency.
- **Unified Insight**: Correlates "UI breakage" with "Backend type errors" to identify the **Root Cause**.
...
- **Autonomous DevLoop**: Agents autonomously perform fix-verify cycles between each other.
